...
**核心主張**：小人口下 LC 的三個參數因**不同機制**失效，而每一種失效都在
**原始資料中留有可事前辨認的徵兆**；因此配適模型之前就能判斷哪些參數可估、
該用哪個估計量。

| 參數 | 病因 | 事前徵兆 | 對策 |
|---|---|---|---|
| $\alpha_x$ | 零格替代與對數轉換 | 診斷②（零格落在哪些年齡） | Firth 偏誤減少 |
| 漂移項 | 等權重分解對異質變異敏感 | 診斷①③（死亡數量級、噪音跨度） | 資訊加權／卜瓦松 |
| $\beta_x$ | 訊噪比不足（BBP 相變） | 診斷④（$s_1$ 對噪音上緣） | 經驗貝氏收縮；門檻下不估 |

四個研究問題依序為：診斷能否事前指出可估性、三個參數的機制是否確實不同、
依機制選用的估計量各能改善多少、以及小人口的區間誤差來自何處。
...
output/
  figures/   所有 .png
  tables/    所有 .csv
報告/         LaTeX 報告（0929 版以前為歷次進度；1006 版為重組後的主線），
              見該目錄的 README
