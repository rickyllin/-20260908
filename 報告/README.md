...
| `進度報告_v2.tex` → `.pdf` | **第二版**，36 頁。增補最佳化演算法的層次與可用性分析（§柒之九） |
| `進度報告_0929.tex` → `.pdf` | **第三版（現行）**，42 頁。依 0922 指導增補資料診斷（§肆之三）、偏誤來源分解（§陸之七）與估計後校正（§陸之八） |
...
> **版本關係**：`進度報告.tex` 為 2026-09-21 會議當天版本，內容凍結；
> `進度報告_v2.tex` 增補演算法分析；`進度報告_0929.tex` 為現行版本，
> 依 0922 指導改為自資料診斷出發，並修正了早期「小樣本的問題是偏誤而非
> 變異數」的通則陳述（該說法對 $\alpha_x$ 與漂移項成立，對 $\beta_x$ 不成立）。
