...
{\zihao{5} （學生姓名）\quad 指導教授：余清祥\,教授}\\[4pt]
...
的共同建模}，如 Li and Lee（2005）的共同 LC 模型、Pitt et al. 的雙性別
...
既有對策大致可歸為兩類。第一類是\textbf{擴大有效樣本}：Li and Lee（2005）
...
\textbf{修勻參數的修勻}則是另一條路線。Basellini et al.（2023）將其分為
...
\subsubsection{（三）尚未處理的缺陷}

綜合上述，本文認為現有文獻仍有四項缺口。

\textbf{缺口一：小人口 LC 模型的系統性研究仍屬少見。}
Basellini et al.（2023）在整份回顧中，僅能舉出 Li et al.（2004）作為「極少數
針對資料有限之族群」的應用，且該文處理的是不等間距的死亡率序列，而非小暴露
數本身。國內雖有 Wang et al.（2018）、Yue et al.（2019, 2021）等一系列研究，
但暴露數門檻的系統性描述仍有補充空間。

\textbf{缺口二：修勻壓低預測區間寬度的現象已被指出，卻無人量化。}
如前所述，Basellini et al.（2023）在討論節明確提出此疑慮並點名五齡組，但未
提供任何量化證據。

\textbf{缺口三：平滑參數的選擇準則與估計目標錯置。}
Delwarde et al.（2007）以交叉驗證選取平滑參數，Rabbi and Mazzuco（2021）
則以對觀測值的誤差比較各種修勻法。前者衡量的是曲面的預測誤差，後者更是
必然選到「不修勻」——因為修勻依定義必然增加對觀測值的誤差。兩者瞄準的都
不是真正關心的估計量。

\textbf{缺口四：時間參數 $\kappa_t$ 在小人口的行為尚未被檢驗。}
...
$\LCe$ 為 Rabbi and Mazzuco（2021）的復刻對象，M$\LCe$ 則為本文額外設計
...
\subsubsection{（一）LASSO 修勻的配適精確度優於樣條}
...
本文於第陸節之六另行設計「配適期長度 $\times$ 人口規模」的二因子模擬，
以固定其一、變動其二的方式回應此一問題。
...
改善；若來自對數轉換本身，則須改換估計架構。本文於第陸節之七以
...
本文於第陸節之五設計四個層層疊加的區間變體（僅未來創新、插入式、
...
本小節針對缺口二進行檢驗，並回應前一小節留下的第二個環節：偏誤如何傳導至
...
$N=5$ 萬時的 0.20 降至 $N=20$ 萬時的 0.03。實務上建議以第伍節所述的
參數式拔靴插入式選法選取，而非固定比例。
...
本文以 Rabbi and Mazzuco（2021）為起點，在臺灣五齡組死亡資料上完成復刻並
向外延伸，主要結論如下。
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{模擬規模已統一}：本版所有蒙地卡羅一律重複 100 次、
        ...
        第伍節的數值因此與早期版本略有出入，其中 $\lambda$ 選法的相對損失
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{補跑 2001$\sim$2019 年作為對照}，以分離 COVID-19 的結構性
        衝擊。
  \item \textbf{以單齡組資料重新檢驗}：分離「五齡組使 $\edag$ 的離散近似
        誤差偏大」與「臺灣死亡轉型較快」這兩個競爭解釋（見
        \S\ref{sec:window}）。
  ...
  \item \textbf{懲罰強度的自動選取}：本文顯示最適 $\lambda/\lambda_{\max}$
        隨人口規模由 0.20 降至 0.03，固定比例並不可行。擬將第伍節的參數式
...
\begin{itemize}[leftmargin=2em, label={}, itemsep=3pt]
  ...
  \item[] Pitt, D., J. Li, and T. K. Lim. ``Smoothing Poisson Common Factor
        Model for Projecting Mortality Jointly for Both Sexes.''
        \textit{ASTIN Bulletin}.（卷期頁碼待補）
