...
\fancyhead[R]{\small 碩士論文進度報告（0929）}
...
{\zihao{5}\textbf{第三版（0929）}：增補資料診斷、偏誤來源分解與估計後校正}\\[12pt]
...
近期的短配適期，且預測區間反而最寬。蒙地卡羅模擬進一步指出，
小樣本下 LC 模型的失效並非單一機制：T-ratio 診斷顯示 $\alpha_x$ 為偏誤
問題（$|T|$ 達 56），$\beta_x$ 則先受變異數主導、大樣本才轉為偏誤；
階梯式分解進一步指出對數轉換僅能解釋 $\alpha_x$ 的偏誤，漂移項的偏誤
另有來源，即主奇異向量對噪音的敏感性。惟文獻長期主張之「LC 模型預測區間過窄」
在此仍不成立。將懲罰項加於年齡參數後比較 LASSO、Ridge 與 Elastic Net，
發現識別條件使收縮目標必須取非零向量，否則懲罰項退化為常數。
\par\vspace{6pt}
...
\textbf{惟此一結論須依參數分別陳述}（詳見第肆節之三的 T-ratio 診斷）：
...
此與第陸節之四所觀察到的「加入參數不確定性反而使涵蓋率下降」是同一件事的
兩種表現。
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{穩健估計的延伸}：以 Huber M-估計量銜接平方損失與 $L_1$ 損失，
        ...
        第柒節之九已完成兩種損失的演算法比較。
  \item \textbf{加權 SVD 的系統評估}：第肆節之四顯示以死亡數加權可使
        SSE($\beta$) 降低近八成、漂移項偏誤減半，惟本文僅作單點比較，
        宜與卜瓦松最大概似並列完整評估。
