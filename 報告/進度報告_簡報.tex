...
\midrule
10,000 & 214.0 & \hi{$+0.439$} & $-0.240$ / $-0.441$ & $-0.110$\\
...
\begin{frame}{陸之二：修勻參數 $\lambda$ 的選擇}
\footnotesize\textcolor{gray}{回應：既然修勻有效，$\lambda$ 該怎麼選？
現行準則選得對嗎？}\normalsize
\vspace{3pt}

下表為四種選法與「已知真值」的 oracle 對照（重複 100 次）。
\textbf{相對損失} $=$ 該選法的 SSE $/$ oracle 的 SSE，
\gd{愈接近 1 代表愈接近最適}：
\vspace{3pt}
\begin{center}\small
...
$N$ & $\lambda$ & SSE($\beta$) & SSE($\beta$) & 相對損失 & SSE($\beta$) & 相對損失\\
\midrule
5 萬   & 40 & 0.0253 & 0.0881 & \hi{3.49} & 0.0531 & \gd{2.10}\\
20 萬  & 20 & 0.0062 & 0.0175 & 2.82 & 0.0065 & \gd{1.05}\\
100 萬 & \phantom{0}5 & 0.0036 & 0.0038 & \gd{1.07} & 0.0039 & 1.09\\
\bottomrule
\end{tabular}
\end{center}
\vspace{4pt}
\begin{itemize}\footnotesize
  \item \textbf{交叉驗證在最需要它的地方失效}：大人口命中（1.07），
        小人口\hi{修勻不足 3.5 倍}。原因是 CV 衡量\textbf{曲面}的預測誤差，
        而 $\beta$ 是曲面的\textbf{泛函}、對雜訊敏感得多。
  \item $\lambda=0$ \hi{根本無法被交叉驗證}（沒有修勻就對留出格產生不了預測）
        $\Rightarrow$ 回答不了「要不要修勻」。
  \item 改用\gd{參數式拔靴的插入式選法}明顯較好，$N=20$ 萬時幾乎追平 oracle。
\end{itemize}
\end{frame}

\begin{frame}{陸之三：三個方向的修勻參數應分開設定}
\footnotesize\textcolor{gray}{回應：原文以單一 $\lambda$ 連動三個方向，
拆開後是否有差？}\normalsize
\vspace{2pt}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
\centering\small
$N=2\times10^5$，重複 100 次
\vspace{3pt}
\begin{tabular}{cccrr}
\toprule
$\lambda_{xx}$ & $\lambda_{xt}$ & $\lambda_{tt}$ & SSE($\beta$) & SSE($\alpha$)\\
\midrule
20 & 5 & 5  & \gd{0.0054} & \hi{4.380}\\
20 & 5 & 20 & 0.0056 & 4.432\\
20 & 5 & 1  & 0.0059 & 4.476\\
\gd{1} & \gd{5} & \gd{5} & \gd{0.0072} & \gd{0.196}\\
0  & 0 & 0  & 0.0540 & 0.224\\
20 & 0 & 0  & \hi{0.1135} & 6.157\\
\bottomrule
\end{tabular}
\end{column}
\begin{column}{0.46\textwidth}
\begin{itemize}\footnotesize
  \item \hi{$\lambda_{xt}$ 是主力}：前八名有七名取到格點上限 $\lambda_{xt}=5$，
        最差三名全部 $\lambda_{xt}=0$。
  \item 純年齡方向的修勻 $(20,0,0)$ 比\textbf{完全不修勻還糟一倍}。
  \item $\lambda_{xx}$ 大會\textbf{毀掉 $\alpha$}（SSE 由 0.2 跳到 4--6）。
  \item 「$\alpha$ 與 $\beta$ 要相反的 $\lambda$」是三方向耦合的\textbf{假象}：
        $(1,5,5)$ 的 SSE($\beta$) 只差三成，SSE($\alpha$) 卻好 \gd{22 倍}。
\end{itemize}
\vspace{4pt}
\footnotesize
\textbf{為何交叉項最有效}：在 LC 底下
...
\begin{frame}{陸之四：預測區間的來源分解\quad\small（$h=10$，名目 80\%）}
\begin{center}\small
\begin{tabular}{rlcc}
\toprule
$N$ & 變體 & 平均寬度 & 涵蓋率\\
...
\begin{frame}{陸之五：配適期長度 $T$ 比人口規模 $N$ 更關鍵}
...
\begin{frame}{柒之九：小結——懲罰項該怎麼用}
\begin{enumerate}\setlength{\itemsep}{9pt}
  \item \textbf{收縮目標必須非零}，否則在 $\mathbf{1}_A^{\!\top}
        \boldsymbol{\beta}=1$ 之下懲罰項退化為常數。
  \item \textbf{若目標是估計精確度}，Ridge 或 Elastic Net 優於 LASSO——
        稀疏性在此問題上並非正確的先驗。
  \item \textbf{若目標是辨識關鍵年齡組}，LASSO 仍是唯一選擇，
        與均方誤差之間的權衡約為 20\%。
  \item \hi{$\lambda$ 必須隨人口規模調整}，固定比例在大人口反而有害
        （最適比例由 $N=5$ 萬的 0.20 降至 20 萬的 0.03）。
  \item \gd{懲罰在 $N\le20$ 萬時效益最大}——此區間正是我國多數鄉鎮市區的
        規模，亦即本方法的實務適用範圍。
\end{enumerate}
\end{frame}

\begin{frame}{附錄：懲罰前後 $\beta_x$ 的逐年齡比較}
\begin{columns}[T]
...
\vspace{4pt}\scriptsize
\textbf{偏誤最大的年齡組（$N=5$ 萬）}
\begin{tabular}{lrrr}
\toprule
年齡 & 真值 & LASSO & 偏誤\\
\midrule
1--4   & .1107 & .0461 & \hi{$-.0646$}\\
100+   & .0058 & .0455 & \hi{$+.0397$}\\
5--9   & .0785 & .0455 & $-.0330$\\
95--99 & .0127 & .0366 & $+.0239$\\
\bottomrule
\end{tabular}
\vspace{6pt}

\footnotesize
\hi{1--4 歲的未懲罰估計值最大（0.107）卻被完全併入 $1/A$。}
軟閾值為 $\lambda w_x/W_x$，而 $W_x=\sum_t\hat\mu_{xt}\kappa_t^2$
是該年齡的\textbf{資訊量}——幼兒死亡數極少使 $W_x$ 很小，
估計值雖大卻不精確，反受最重的懲罰。
\vspace{4pt}

\gd{懲罰強弱取決於資訊量而非係數大小}；
存活下來的是 45--54 與 60--99 歲。
\end{column}
\end{columns}
\vspace{2pt}
\footnotesize
標準差中位數降 \gd{75\%}，與絕對偏誤增為 1.8 倍之間取得平衡；
偏誤集中在\textbf{兩端年齡}，正是未懲罰標準差高達 0.10--0.19、本就不堪用之處。
\end{frame}
...
\begin{enumerate}\small\setlength{\itemsep}{7pt}
  ...
  \item \textbf{平滑參數的選擇準則與估計目標錯置。}
        ...
        前者小人口修勻不足 3.5 倍、後者在台灣數據上傾向選擇不修勻。
...
\begin{frame}{下一步工作}
\footnotesize
\textcolor{gray}{（統一模擬規模與 $T\times N$ 二因子設計已於本次完成，見陸之五）}
\vspace{4pt}
\begin{enumerate}\small\setlength{\itemsep}{7pt}
  \item \textbf{COVID-19 對照組}：補跑 2001--2019 年並列，
        分離結構性衝擊的影響。
  \item \textbf{量化初始值的取捨}：Stoeldraijer et al.\ (2018) 指出初始值的
        選擇存在準確度與穩健度之間的權衡；本文顯示這是\hi{殘餘涵蓋誤差的
        主要來源}（$N=20$ 萬時寬度已對、涵蓋仍僅 0.63）。
  \item \textbf{假設母體的敏感度分析}：以受控偏離
        $\boldsymbol{\beta}^{\text{true}}(\delta)=(1-\delta)
        \hat{\boldsymbol{\beta}}^{\text{全國}}+\delta\mathbf{v}$，
        檢驗三個方向——\textbf{甲} 高齡改善較快、\textbf{乙} 幼年改善較快、
        \textbf{丙} 中年反轉。判準：\gd{結論在 $\delta\le0.3$ 內維持不變
        即可宣稱穩健}。
  \item \textbf{壓力測試}：$\kappa_t$ 跳躍（結構性衝擊）、$\beta_x$ 緩慢旋轉
        （違反 LC 核心假設）、負二項過度離散、人口降至 5{,}000，
        以界定\hi{「在何種條件下本方法不應使用」}。
  \item \textbf{懲罰強度的自動選取}，以及自適應權重與收斂理論的補強。
\end{enumerate}
\end{frame}

\begin{frame}{請老師指教}
\vspace{4pt}
\begin{block}{想請教的四個方向}
\begin{enumerate}\setlength{\itemsep}{8pt}
  ...
  \item 假設母體的\textbf{敏感度分析}要做到什麼程度才足夠？
        目前規劃以受控偏離檢驗三個方向（高齡改善較快、幼年改善較快、中年反轉）。
\end{enumerate}
\end{block}
\vspace{6pt}
\begin{center}
