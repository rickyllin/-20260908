...
\author[（學生姓名）]{（學生姓名）\\[4pt]\small 指導教授：余清祥\,教授}
...
\end{block}
\vspace{2pt}
\centerline{\small\textbf{本文即由此二缺口出發。}}
\end{frame}
...
\begin{enumerate}\setlength{\itemsep}{12pt}
  \item \textbf{問題一}：Rabbi and Mazzuco (2021) 在國家級資料上成立的優勢，
        ...
        \\[2pt]\footnotesize\textcolor{gray}{$\rightarrow$ 第 4--9 張，以實證復刻回應}
...
\begin{enumerate}\setlength{\itemsep}{10pt}
  \item \textbf{復刻}：Rabbi and Mazzuco (2021) 的 $\LCe$ 方法在臺灣五齡組資料上
...
BMS & 卜瓦松離差 ＋ 搜尋配適起始年\\
\hi{$\LCe$} & \hi{2D L1 修勻 $\to$ 對齊 $\edag$}（復刻對象）\\
...
\begin{frame}{復刻過程浮現的三個方法論問題}
\begin{block}{(1) 原文 Fig.~8 的相關係數是\hi{結構性假象}}
...
\end{block}
\begin{block}{(3) 原文選修勻參數的準則\hi{必然選到「不修勻」}}
...
\begin{enumerate}\small\setlength{\itemsep}{7pt}
  \item \textbf{小人口 LC 模型的系統性研究仍屬少見。}
        Basellini et al.\ (2023) 全篇僅能舉出 Li et al.\ (2004) 作為
        「極少數針對資料有限之族群」的應用，且處理的是不等間距序列而非小暴露數。
...
\end{enumerate}
\vspace{4pt}
\centerline{\small 本文針對\textbf{缺口二至四}提出量化證據。}
\end{frame}
...
\begin{enumerate}\setlength{\itemsep}{8pt}
  \item 論文主軸要放在\textbf{缺口二}（修勻壓低區間寬度）
        還是\textbf{缺口四}（$\kappa_t$ 的偏誤而非變異）？
